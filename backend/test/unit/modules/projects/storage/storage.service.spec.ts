...
import { StorageQueryService } from '@app/modules/projects/storage/storage-query.service';
import { StorageService } from '@app/modules/projects/storage/storage.service';
import { StorageUploadService } from '@app/modules/projects/storage/storage-upload.service';
...
describe('StorageService', () => {
  ...
  beforeEach(() => {
    ...
    minioStorageService.getBucketName.mockReturnValue('educodeai-storage');
  ...
  it('debe subir objeto y persistir metadata cuando todo es valido', async () => {
    ...
    expect(minioStorageService.putObject).toHaveBeenCalledWith({
      ...
      body: file.buffer,
      contentType: 'text/x-python',
      // se envía siempre la longitud. Con un `Buffer` es
